...
RSpec.describe "match_schema" do
  ...
  it "passes a conforming payload" do
    expect(valid).to match_schema(schema)
  end

  it "fails a non-conforming payload" do
    expect(valid.merge("id" => "1")).to_not match_schema(schema)
  end

  it "names the failing pointer in the message" do
    expect {
      expect(valid.merge("id" => "1")).to match_schema(schema)
  ...
  it "names an undeclared key" do
    expect {
      expect(valid.merge("created_at" => "2026-09-11")).to match_schema(schema)
