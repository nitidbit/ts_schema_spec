...
RSpec.describe "match_ts_schema(path, type)" do
  let(:role) { { "id" => 1, "name" => "Nurse", "shortcode" => nil } }

  it "takes the source and type directly" do
    expect(role).to match_ts_schema(fixture("roleMatrix.ts"), "Role")
  end

  it "fails a non-conforming payload" do
    expect(role.merge("id" => "1")).to_not match_ts_schema(fixture("roleMatrix.ts"), "Role")
  end

  it "names the type and the source in the failure message" do
    expect {
      expect(role.merge("id" => "1")).to match_ts_schema(fixture("roleMatrix.ts"), "Role")
